...
T_GAP_DEV_STATE bt_mesh_provisioner_multiple_profile_gap_dev_state = {0, 0, 0, 0, 0};                 /**< GAP device state */
#if defined(CONFIG_BT_MESH_CENTRAL) && CONFIG_BT_MESH_CENTRAL
...
void bt_mesh_provisioner_multiple_profile_app_handle_gap_msg(T_IO_MSG *p_gap_msg);
...
void bt_mesh_provisioner_multiple_profile_app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_BT_STATUS:
        {
            bt_mesh_provisioner_multiple_profile_app_handle_gap_msg(&io_msg);
        }
        break;
    ...
    case IO_MSG_TYPE_ADV:
    {
        ...
        rtw_memcpy(padv_data, adv_data, sizeof(adv_data));    
 ...
 */
#if defined(CONFIG_BT_MESH_SCATTERNET) && CONFIG_BT_MESH_SCATTERNET
extern void sw_bt_info_update(u8 is_scan);
#endif

void bt_mesh_provisioner_multiple_profile_app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
    APP_PRINT_INFO4("bt_mesh_provisioner_multiple_profile_app_handle_dev_state_evt: init state  %d, adv state %d, scan state %d, cause 0x%x",
    ...
    (defined(CONFIG_BT_MESH_SCATTERNET) && CONFIG_BT_MESH_SCATTERNET))
    if (bt_mesh_provisioner_multiple_profile_gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
...
#if ((defined(CONFIG_BT_MESH_PERIPHERAL) && CONFIG_BT_MESH_PERIPHERAL) || \
    (defined(CONFIG_BT_MESH_SCATTERNET) && CONFIG_BT_MESH_SCATTERNET))
    if (bt_mesh_provisioner_multiple_profile_gap_dev_state.gap_adv_state != new_state.gap_adv_state)
    {
    ...
    if (bt_mesh_provisioner_multiple_profile_gap_dev_state.gap_scan_state != new_state.gap_scan_state)
    {
    ...
    bt_mesh_provisioner_multiple_profile_gap_dev_state = new_state;
}
 ...
 * @return   void
 */
void bt_mesh_provisioner_multiple_profile_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
#if defined(CONFIG_BT_MESH_SCATTERNET) && CONFIG_BT_MESH_SCATTERNET    
    ...
    APP_PRINT_INFO4("bt_mesh_provisioner_multiple_profile_app_handle_conn_state_evt: conn_id %d, conn_state(%d -> %d), disc_cause 0x%x",
    ...
    case GAP_CONN_STATE_DISCONNECTED:
        {
            ...
            {
                APP_PRINT_ERROR2("bt_mesh_provisioner_multiple_profile_app_handle_conn_state_evt: connection lost, conn_id %d, cause 0x%x", conn_id,
                                 disc_cause);
            }
 ...
 * @return   void
 */
void bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt(uint8_t conn_id, uint8_t new_state, uint16_t cause)
{
    APP_PRINT_INFO2("bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt:conn_id %d, cause 0x%x", conn_id, cause);

    switch (new_state)
    {
    case GAP_AUTHEN_STATE_STARTED:
        {
            APP_PRINT_INFO0("bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt: GAP_AUTHEN_STATE_STARTED");
        }
        break;
    ...
    case GAP_AUTHEN_STATE_COMPLETE:
        {
            if (cause == GAP_SUCCESS)
            {
                ...
                APP_PRINT_INFO0("bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair success");

            }
            else
            {
                data_uart_debug("Pair failed: cause 0x%x\r\n", cause);
                APP_PRINT_INFO0("bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair failed");
            }
        }
        break;
    ...
    default:
        {
            APP_PRINT_ERROR1("bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt: unknown newstate %d", new_state);
        }
        break;
    }
}
 ...
 * @return   void
 */
void bt_mesh_provisioner_multiple_profile_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
 ...
 * @return   void
 */
void bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
    switch (status)
    {
    case GAP_CONN_PARAM_UPDATE_STATUS_SUCCESS:
        {
            ...
            le_get_conn_param(GAP_PARAM_CONN_TIMEOUT, &conn_supervision_timeout, conn_id);
            APP_PRINT_INFO4("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update success:conn_id %d, conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x",
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_FAIL:
        {
            APP_PRINT_ERROR2("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update failed: conn_id %d, cause 0x%x",
                             conn_id, cause);
        }
        break;
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_PENDING:
        {
            APP_PRINT_INFO1("bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt update pending: conn_id %d", conn_id);
        }
        break;
...
void bt_mesh_provisioner_multiple_profile_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
    ...
    APP_PRINT_TRACE1("bt_mesh_provisioner_multiple_profile_app_handle_gap_msg: sub_type %d", p_gap_msg->subtype);
    ...
    case GAP_MSG_LE_DEV_STATE_CHANGE:
        {
            ...
            bt_mesh_provisioner_multiple_profile_app_handle_dev_state_evt(gap_msg.msg_data.gap_dev_state_change.new_state,
    ...
    case GAP_MSG_LE_CONN_STATE_CHANGE:
        {
            bt_mesh_provisioner_multiple_profile_app_handle_conn_state_evt(gap_msg.msg_data.gap_conn_state_change.conn_id,
    ...
    case GAP_MSG_LE_CONN_MTU_INFO:
        {
            bt_mesh_provisioner_multiple_profile_app_handle_conn_mtu_info_evt(gap_msg.msg_data.gap_conn_mtu_info.conn_id,
    ...
    case GAP_MSG_LE_CONN_PARAM_UPDATE:
        {
            bt_mesh_provisioner_multiple_profile_app_handle_conn_param_update_evt(gap_msg.msg_data.gap_conn_param_update.conn_id,
    ...
    case GAP_MSG_LE_AUTHEN_STATE_CHANGE:
        {
            bt_mesh_provisioner_multiple_profile_app_handle_authen_state_evt(gap_msg.msg_data.gap_authen_state.conn_id,
...
T_APP_RESULT bt_mesh_scatternet_gap_service_callback(T_SERVER_ID service_id, void *p_para)
{
    ...
    if (p_gap_data->msg_type == SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE)
    {
        ...
        default:
			...
			//data_uart_print("bt_mesh_scatternet_gap_service_callback: unhandled msg_data.opcode 0x%x", p_gap_data->msg_data.opcode);
            break;
        }
    }
    return result;
...
{
    T_APP_RESULT  result = APP_RESULT_SUCCESS;
    APP_PRINT_INFO2("bt_mesh_provisioner_multiple_profile_app_client_callback: client_id %d, conn_id %d",
                    client_id, conn_id);
