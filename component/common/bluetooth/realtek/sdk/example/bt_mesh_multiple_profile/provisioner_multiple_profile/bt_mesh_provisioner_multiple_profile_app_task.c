 ...
 * @return void
 */
void bt_mesh_provisioner_multiple_profile_app_task_init()
{
    os_task_create(&bt_mesh_provisioner_multiple_profile_app_task_handle, "bt_mesh_provisioner_multiple_profile_app", bt_mesh_provisioner_multiple_profile_app_main_task, 0, APP_TASK_STACK_SIZE,
                   APP_TASK_PRIORITY);
}
...
void bt_mesh_provisioner_multiple_profile_app_main_task(void *p_param)
{
    ...
    while (true)
    {
        if (os_msg_recv(bt_mesh_provisioner_multiple_profile_evt_queue_handle, &event, 0xFFFFFFFF) == true)
        {
            if (event == EVENT_IO_TO_APP)
            {
                ...
                {
                    bt_mesh_provisioner_multiple_profile_app_handle_io_msg(io_msg);
                }
            }
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
...
{
	if (bt_mesh_provisioner_multiple_profile_io_queue_handle) {
		os_msg_queue_delete(bt_mesh_provisioner_multiple_profile_io_queue_handle);
	}
	if (bt_mesh_provisioner_multiple_profile_evt_queue_handle) {
		os_msg_queue_delete(bt_mesh_provisioner_multiple_profile_evt_queue_handle);
	}
