 ...
 *                              Variables
 *============================================================================*/
void *bt_mesh_device_app_task_handle;   //!< APP Task handle
void *bt_mesh_device_evt_queue_handle;  //!< Event queue handle
void *bt_mesh_device_io_queue_handle;   //!< IO queue handle

/*============================================================================*
 *                              Functions
 *============================================================================*/
void bt_mesh_device_app_main_task(void *p_param);
...
void bt_mesh_send_io_msg(T_IO_MSG *p_io_msg)
{
    ...
    if (os_msg_send(bt_mesh_device_io_queue_handle, p_io_msg, 0) == false)
    {
 ...
 * @return void
 */
void bt_mesh_device_app_task_init()
{
    os_task_create(&bt_mesh_device_app_task_handle, "bt_mesh_device_app", bt_mesh_device_app_main_task, 0, APP_TASK_STACK_SIZE,
                   APP_TASK_PRIORITY);
}
...
void bt_mesh_device_app_main_task(void *p_param)
{
    ...
    os_msg_queue_create(&bt_mesh_device_io_queue_handle, MAX_NUMBER_OF_IO_MESSAGE, sizeof(T_IO_MSG));
    os_msg_queue_create(&bt_mesh_device_evt_queue_handle, MAX_NUMBER_OF_EVENT_MESSAGE, sizeof(uint8_t));
    gap_start_bt_stack(bt_mesh_device_evt_queue_handle, bt_mesh_device_io_queue_handle, MAX_NUMBER_OF_GAP_MESSAGE);

    mesh_start(EVENT_MESH, EVENT_IO_TO_APP, bt_mesh_device_evt_queue_handle, bt_mesh_device_io_queue_handle);
    ...
    while (true)
    {
        if (os_msg_recv(bt_mesh_device_evt_queue_handle, &event, 0xFFFFFFFF) == true)
        {
            if (event == EVENT_IO_TO_APP)
            {
                T_IO_MSG io_msg;
                if (os_msg_recv(bt_mesh_device_io_queue_handle, &io_msg, 0) == true)
                {
                    bt_mesh_device_app_handle_io_msg(io_msg);
                }
            }
#if defined(CONFIG_BT_MESH_USER_API) && CONFIG_BT_MESH_USER_API
...
{
	if (bt_mesh_device_io_queue_handle) {
		os_msg_queue_delete(bt_mesh_device_io_queue_handle);
	}
	if (bt_mesh_device_evt_queue_handle) {
		os_msg_queue_delete(bt_mesh_device_evt_queue_handle);
	}
	if (bt_mesh_device_app_task_handle) {
		os_task_delete(bt_mesh_device_app_task_handle);
	}
	bt_mesh_device_io_queue_handle = NULL;
	bt_mesh_device_evt_queue_handle = NULL;
	bt_mesh_device_app_task_handle = NULL;
